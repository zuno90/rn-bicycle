...
const CategoryBlock: React.FC = () => {
  ...
  const [takeCate, setTakeCate] = React.useState<ICategory[]>([])
  const getCategories = async () => {
    ...
    if (res.success) {
      ...
      setTakeCate(res.data.categories.slice(0, 7))
    }
  }
  ...
  return (
    <Box maxW={WIDTH} flexDir="row" flexWrap="wrap" alignItems="center" gap={1.5}>
      {categories.length > 0 &&
        takeCate.map((item, index) => (
          <Box key={index} flexDir="column" maxW="24%" alignItems="center" gap={2}>
            <Pressable
              onPress={() =>
            ...
            <Text h={WIDTH * 0.1} fontSize="sm" textAlign="center" numberOfLines={2} isTruncated>
              {item.name}
            </Text>
          </Box>
        ))}

      {categories.length > takeCate.length && (
        <Box
          flexDir="column"
          maxW="24%"
          overflow="hidden"
          ...
          gap={2}
        >
          <Pressable onPress={() => setTakeCate(categories)}>
            <Box
              size={WIDTH * 0.23}
